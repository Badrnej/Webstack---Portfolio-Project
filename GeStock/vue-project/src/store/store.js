...
const store = createStore({
  ...
  actions: {
    ...
    async signUp(context, user) {
      ...
      try {
        // Add role: 'admin' to force admin registration
    ...
    async updateClient({ commit }, clientData) {
      try {
        const response = await axios.post(
          `http://127.0.0.1:8000/api/client/update/${clientData.id}`,
          clientData,
          {
            headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` }
          }
    ...
    async updateFournisseur({ commit, state }, fournisseurData) {
      try {
        const response = await axios.post(
          `http://127.0.0.1:8000/api/fournissur/update/${fournisseurData.id}`,
          fournisseurData,
          {
            headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` }
          }
    ...
    async updateFacture({ commit, state }, factureData) {
      try {
        const response = await axios.post(
          `http://127.0.0.1:8000/api/facture/update/${factureData.id}`,
          factureData,
          {
            headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` }
          }
